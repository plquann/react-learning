...
export default class RenderCardMovie extends Component {
    dataMovies = [
        {
            "maPhim": 1321,
            "tenPhim": "Ted2",
    ...
    render() {
        return (
            <div className="container">
                <h3 className="text-center display-4">List Movies</h3>
                <div className="row">
                    {this.dataMovies.map((movie, index) => {
                        return <div className="col-4 mb-5" key={index}>
                            <div className="card">
                                <div style={{
                                    backgroundImage: `url('${movie.hinhAnh}'), url('https://picsum.photos/150/450')`,
                                    backgroundPosition: 'center', backgroundSize: 'cover'
                                }}>
                                    <img style={{ opacity: 0 }} src={movie.hinhAnh} className="card-img-top" width="150" height="450" alt={movie.hinhAnh} />
                                </div>
                                <div className="card-body">
                                    <h5 className="card-title" style={{ height: 50 }}>{movie.tenPhim.length > 50 ? <p>{movie.tenPhim.sub(0, 100)} ...</p> : <p>{movie.tenPhim}</p>}</h5>
                                    <div className="card-text" style={{ height: 80 }}>{movie.moTa.length > 100 ? <p>{movie.moTa.substr(0, 100)} ... </p> : <p>{movie.moTa}</p>}</div>
                                </div>
                            </div>
                        </div>
                    })}
